...
6. Generate the configured number of reviewer reports with different emphasis.
   - Use the user-requested count when supplied; otherwise default to `3`.
   - Use the same fact base for every report.
...
8. Run final QA.
   - Check evidence anchors, issue-key normalization, synthesis consistency,
     groundedness, configured-count coverage, and non-invention.
