...
## Weighting guidance across reports

- Use the user-requested reviewer count when supplied; otherwise default to
  three reports.
- In the default configuration, `Reviewer 1` usually foregrounds `technical
  soundness / technical failings`; `Reviewer 2` foregrounds `originality` plus
  `scientific importance / significance`; and `Reviewer 3` foregrounds
  `interdisciplinary readership interest` plus `readability for
  nonspecialists`.
- For another count, distribute those emphasis families across the configured
  reports without inventing new personas or artificial disagreement.
- Every report should still cover all core axes briefly; the difference is
  weight, not scope omission.
