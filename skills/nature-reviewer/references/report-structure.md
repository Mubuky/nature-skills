...
- Use the user-requested reviewer count when supplied; otherwise use `N = 3`.
- The output should contain these sections in order:
  1. `Review setup`
  2. `Reviewer 1` through `Reviewer N`
  3. `Cross-review synthesis`
  4. `Risk / unsupported claims`
...
- List a concern under `Consensus technical risks` only when at least two
  reviewer reports map to the same normalized issue key.
- When only one reviewer report was requested, do not claim cross-review
  consensus; use the synthesis as a concise overall readout.
