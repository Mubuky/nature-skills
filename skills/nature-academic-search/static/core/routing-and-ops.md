...
Provide Semantic Scholar / NCBI keys through the host's environment or secret
store. Do not commit `.env` files. Elsevier keys are read from the local
pybliometrics config, normally `~/.config/pybliometrics.cfg`; do not copy API
keys into this skill.
...
The format converter (`scripts/format-converter.py`) uses Python stdlib only —
no extra dependencies. Run
`python3 "$SKILL_DIR/scripts/format-converter.py" --test` to verify it.
...
python3 "$SKILL_DIR/scripts/academic_search.py" "graph neural network potentials" --limit 10 --sort cited_by_count --mailto you@example.com
python3 "$SKILL_DIR/scripts/format-converter.py" --doi 10.1103/physrevlett.120.143001 --format ris
```
