...
2. **Download** via format-converter:
   ```bash
   # PubMed
   python3 "$SKILL_DIR/scripts/format-converter.py" --pmid 28344011 --format nbib

   # CrossRef
   python3 "$SKILL_DIR/scripts/format-converter.py" --doi 10.1038/nature14539 --format ris

   # arXiv
   python3 "$SKILL_DIR/scripts/format-converter.py" --arxiv 1706.03762 --format bib

   # Batch from file
   python3 "$SKILL_DIR/scripts/format-converter.py" --input refs.txt --format ris
   ```
