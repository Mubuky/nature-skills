...
For automation, use structured standard input and never place secrets in
command arguments, replies, or manifests:
...
Otherwise prefer local hidden input. Optional Elsevier tokens and the IEEE
Full-Text endpoint template also use the hidden prompt or stdin JSON, never
argv. Institutional passwords, OTP codes, cookies, and session tokens are not
handled by this rule. Secrets are stored in
`~/.config/lit-dl/credentials.json` with owner-only permissions; short secrets
are fully masked and longer secrets expose only their last four characters.
...
- CAPTCHA, sliders, Turnstile/reCAPTCHA, robot checks, QR approval, SMS/OTP,
  passkeys, hardware keys, and two-factor authentication always require an
  immediate user handoff. Scripts must not click, drag, solve, or claim to
