...
// Stops at institutional SSO / CARSI pages and never handles credentials or
// interacts with CAPTCHA, slider, robot, OTP, or other security challenges.
...
async function downloadDoi(proxy, doi, outDir, wantSi, debug, { cleanBundle = false, bundleTitle = "" } = {}) {
  ...
  try {
    ...
    if (wall) {
      if (debug) process.stderr.write(`[debug][doi] wall detected: ${wall.status} "${wall.reason}" — user handoff required\n`);
      return { doi, status: wall.status, url: info.url, reason: wall.reason };
    }
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (w) {
        if (debug) process.stderr.write(`[debug][doi] meta-stage wall: ${w.status} "${w.reason}" — user handoff required\n`);
        return { doi, status: w.status, url: meta.url, reason: w.reason };
      }
...
async function routeDoiDownload(doi, args, context) {
  ...
  if (route.provider === "web_access") {
    if (args.apiFallbackWeb === false) {
      return decorateResult({
        ...(oaResult || { status: STATUS.NO_AUTHORIZED_PDF_FOUND }),
        web_fallback: "declined",
      }, article, route, args.si);
    }
    if (args.route !== "web_access" && args.apiFallbackWeb !== true) {
      return decorateResult({
        ...(oaResult || {}),
        status: STATUS.API_FALLBACK_CONFIRMATION_REQUIRED,
        next_action:
          "OA retrieval did not yield full text. Confirm use of the user's authenticated institutional browser, then re-run with --route web_access or --api-fallback-web.",
      }, article, route, args.si);
    }
