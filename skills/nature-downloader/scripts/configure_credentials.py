...
def cmd_set(args: argparse.Namespace) -> int:
    if args.stdin_json:
        try:
            ...
            raise SystemExit(f"stdin JSON 无效: {error}") from error
        if not isinstance(supplied, dict):
            raise SystemExit("stdin JSON 必须是对象")
    else:
        supplied = {
            "api_key": getpass.getpass("API key: "),
            "insttoken": getpass.getpass("Institution token (optional): ")
            if args.provider == "elsevier"
            else "",
            "authtoken": getpass.getpass("Authentication token (optional): ")
            if args.provider == "elsevier"
            else "",
            "fulltext_endpoint": getpass.getpass(
                "IEEE Full-Text endpoint template (optional, hidden): "
            )
            if args.provider == "ieee"
            else "",
        }
    api_key = str(supplied.get("api_key", "")).strip()
    if not api_key:
        raise SystemExit("API key 不能为空")
    values = {"api_key": api_key}
    for name in ("insttoken", "authtoken"):
        value = str(supplied.get(name, "")).strip()
        if value:
            values[name] = value
    endpoint = str(supplied.get("fulltext_endpoint", "")).strip()
    if endpoint:
        values["fulltext_endpoint"] = endpoint
...
def build_parser() -> argparse.ArgumentParser:
    ...
    set_cmd.add_argument(
        "--stdin-json",
        action="store_true",
        help="Read api_key and optional tokens from a JSON object on stdin.",
    )
