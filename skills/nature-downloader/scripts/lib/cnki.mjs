...
function classifyCnkiWall(url = "", title = "", body = "") {
  ...
  // Slider/drag/puzzle CAPTCHA — user handoff only.
  if (/滑块|滑动验证|拖动滑块|拼图验证|请按住滑块|请拖动|slide to verify|drag the slider|slide verification|slider captcha/i.test(text)) {
    return { status: STATUS.PUBLISHER_VERIFICATION_WAITING_USER, reason: "CNKI slider CAPTCHA requires user verification" };
  }
  // Login/auth wall
...
export async function downloadCnkiTitle(proxy, title, outDir, { cnkiUrl = DEFAULT_CNKI_URL, format = "any", debug = false, wantSi = false, transport = null } = {}) {
  ...
  try {
    ...
    if (wall) {
      if (debug) process.stderr.write(`[debug][cnki] wall after search: ${wall.status} "${wall.reason}" — user handoff required\n`);
      return { title, status: wall.status, url: snap.url, reason: wall.reason };
    }
    ...
    if (wall) {
      if (debug) process.stderr.write(`[debug][cnki] wall after detail nav: ${wall.status} "${wall.reason}" — user handoff required\n`);
      return { title, status: wall.status, url: snap.url, reason: wall.reason };
    }
