...
2. **工具**（本技能 `scripts/disclosure/`）：**`cnipa_epub_search.py`** —— 等待公开站点正常 JavaScript 渲染后解析结果；遇到登录、CAPTCHA、WAF 交互挑战或访问限制时立即停止并交由用户或改用其他公开来源。结果页 HTML **仅在内存中处理，不落盘**。成功时终端含 **`EPUB_NOTE:`** 与 **`EPUB_HITS_JSON:`** 一行。
...
5. **`abstract` 字段（国知局条目，规定必用）**
   ...
   - **必用**：查新笔记、交底书 **1.1** 中对该专利的技术方案概括与应用场景，必须先基于对该 `abstract` 的完整阅读与理解后再撰写；禁止仅凭标题、公开号或 URL 臆造方案要点。
   - **充分理解**：在写入 1.1 或查新笔记前，Agent 须在**推理过程内**明确：摘要所涉**技术领域、解决什么问题、核心手段/模块、主要效果或流程**；若摘要与标题存在差异，**以摘要为准**概括该技术。
   - **正文呈现**：交底书 1.1 中不得大段逐字粘贴官方摘要；应压缩为「方案概括 + 应用」。局限性只能标为 `author-stated limitation`，或明确标注为带证据的 `agent analytical inference`；摘要未提及某特征不等于该特征不存在。
...
- 应用场景
- 局限性（区分 `author-stated` 与带证据的 `agent inference`）
...
文末给出：检索总结与“基于已检索材料的初步技术差异，待专业检索复核”。
每次结果须注明：`non-exhaustive technical landscape; not a novelty,
inventiveness, FTO, or legal opinion`。每个差异和 prior-art feature 都要
回指公开源定位；没有全文时不得把摘要中未出现的内容当作不存在。
