...
| **`cnipa_epub_parse.py`** | 仅解析已保存的 HTML：`python3 "$SKILL_DIR/scripts/disclosure/cnipa_epub_parse.py" path/to/_last_result_xxx.html`；字段含标题、公开号、链接、**`abstract`**（若有）。 |
...
mermaid 脚本按顺序查找：`scripts/disclosure/node_modules/.bin/mmdc` →
**PATH** 上的 `mmdc`。若均不存在，脚本停止并报告缺失；它不会通过 `npx`
静默下载依赖。
