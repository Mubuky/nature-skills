...
class DraftValidationTests(unittest.TestCase):
    ...
    def test_unconfirmed_or_unsupported_evidence_cannot_support_claim(self):
        for status in ("needs-confirmation", "unsupported"):
            with self.subTest(status=status):
                draft = valid_draft()
                draft["evidence_ledger"][0]["support_status"] = status
                codes = {item.code for item in VALIDATOR.validate(draft)}
                self.assertIn("CLAIM_EVIDENCE_STATUS", codes)

    def test_inherent_support_requires_rationale(self):
        draft = valid_draft()
        draft["evidence_ledger"][0]["support_status"] = "inherent"
        codes = {item.code for item in VALIDATOR.validate(draft)}
        self.assertIn("INHERENCE_RATIONALE", codes)

        draft["evidence_ledger"][0]["inherence_rationale"] = (
            "The cited equation necessarily performs the mapped fusion step."
        )
        codes = {item.code for item in VALIDATOR.validate(draft)}
        self.assertNotIn("INHERENCE_RATIONALE", codes)

    def test_claim_source_requires_locator(self):
        draft = valid_draft()
        draft["source_map"][0]["locator"] = ""
        codes = {item.code for item in VALIDATOR.validate(draft)}
        self.assertIn("CLAIM_SOURCE_LOCATOR", codes)
