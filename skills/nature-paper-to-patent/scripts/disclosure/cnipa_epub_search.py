...
需已安装：python3 -m pip install -r scripts/disclosure/requirements-cnipa.txt
并运行 python3 -m playwright install chromium

用法：

  python3 scripts/disclosure/cnipa_epub_search.py 词1
  python3 scripts/disclosure/cnipa_epub_search.py "短语 含 空格"
  python3 scripts/disclosure/cnipa_epub_search.py 词甲 词乙 词丙
...
环境变量：与 ``cnipa_epub_crawler.py`` 相同（如
``EPUB_RENDER_MAX_WAIT_SEC``、``PLAYWRIGHT_HEADED``）。
"""
...
def _usage() -> None:
    print("usage: python3 scripts/disclosure/cnipa_epub_search.py <term> [more terms...]", file=sys.stderr)
    print(
        "whitespace splits to multiple terms; one Playwright run per term; merge by pub_number.",
        file=sys.stderr,
    )
    print('example: python3 scripts/disclosure/cnipa_epub_search.py "batch 调度 异构"', file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    os.environ.setdefault("EPUB_RENDER_MAX_WAIT_SEC", "180")
