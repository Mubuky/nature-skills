...
2. **PATH 上的 ``mmdc``**（通常为显式安装的 Mermaid CLI）。

本脚本不会通过 ``npx`` 静默下载依赖。找不到 ``mmdc`` 时会报告缺失，让用户
决定是否安装或改用不渲染 Mermaid 的路线。
...
用法：
  python3 scripts/disclosure/mermaid_render.py -i draft.md -o disclosure.md
  # 默认在同目录生成 disclosure.docx；失败时 stderr 会给出可复制的 md_to_docx 命令
  python3 scripts/disclosure/mermaid_render.py -i draft.md -o out/disclosure.md --docx out/custom.docx
  python3 scripts/disclosure/mermaid_render.py -i draft.md -o disclosure.md --no-docx   # 仅 Markdown
...
def _find_mmdc_invocation() -> tuple[list[str], bool]:
    """Return an installed Mermaid CLI without downloading dependencies."""
    local = _local_mmdc()
    if local:
        return local
    ...
    raise FileNotFoundError(
        "mmdc is not installed. Install Mermaid CLI explicitly or rerun without "
        "Mermaid rendering; this script does not download dependencies."
    )
...
def render_markdown_mermaid(
    ...
    *,
    ...
    try:
        ...
        mmdc_error: str | None = None
    except FileNotFoundError as error:
        mmdc_base, use_shell = [], False
        mmdc_error = str(error)
...
def _print_manual_docx_hint(out_md: Path, docx_out: Path, base_dir: Path, md_script: Path) -> None:
    ...
    if md_script.is_file():
        ...
        print(
            "  python3 scripts/disclosure/md_to_docx.py -i <上述.md> -o <输出.docx> --base-dir <.md 所在目录>",
            file=sys.stderr,
        )
