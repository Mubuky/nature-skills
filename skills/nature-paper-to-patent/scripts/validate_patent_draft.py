...
def validate(data: dict) -> list[Finding]:
    ...
    for item in data.get("evidence_ledger", []):
        ...
        if status == "inherent" and not str(item.get("inherence_rationale", "")).strip():
            add(
                findings,
                "ERROR",
                "INHERENCE_RATIONALE",
                f"{ledger_id}标记为inherent但缺少可审查的必然性理由。",
            )
    ...
    for mapping in data.get("claim_feature_map", []):
        ...
        for evidence_id in evidence_ids:
            if evidence_id not in ledger_ids:
                ...
                continue
            evidence = ledger_by_id[evidence_id]
            status = evidence.get("support_status")
            if status not in {"explicit", "inherent"}:
                add(
                    findings,
                    "ERROR",
                    "CLAIM_EVIDENCE_STATUS",
                    f"权利要求{claim_number}的正式特征引用了"
                    f"{status}证据：{evidence_id}。",
                )
            for source_id in evidence.get("source_ids", []):
                source = source_by_id.get(source_id, {})
                if source_id in source_by_id and not str(source.get("locator", "")).strip():
                    add(
                        findings,
                        "ERROR",
                        "CLAIM_SOURCE_LOCATOR",
                        f"权利要求{claim_number}引用的来源{source_id}缺少定位信息。",
                    )
