...
**R generation rule.** When the user has selected R, keep drawing, layout
semantics, preview generation, and export in R. Do not call
matplotlib/seaborn or a Python graphics device to create a fallback rendering
or layout approximation. Neutral image/PDF/SVG viewers, metadata inspectors,
and the bundled validator may audit R outputs. If `Rscript`/R or required R
packages are missing, stop before rendering and report the exact blocker. You
may still write the R script, provide install commands, or ask permission to
install dependencies, but do not cross-render the figure in Python.
