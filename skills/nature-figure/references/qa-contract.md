<!-- MODIFIED IN THIS DERIVATIVE: validator paths and neutral-QA semantics were made installation-safe; see ../../../NOTICE (Apache-2.0 section 4(b)). -->
...
| Backend consistency | The selected backend produced all plotting, layout decisions, previews, and exports; neutral tools only inspected the products |
...
```bash
SKILL_DIR="/absolute/path/to/the/loaded/nature-figure"

# Python source
python3 "$SKILL_DIR/scripts/validate_figure.py" path/to/figure.py

# R, R Markdown, or Quarto source
python3 "$SKILL_DIR/scripts/validate_figure.py" path/to/figure.R

# Machine-readable report or stricter warning gate
python3 "$SKILL_DIR/scripts/validate_figure.py" path/to/figure.py --json
python3 "$SKILL_DIR/scripts/validate_figure.py" path/to/figure.py --strict
```

Resolve `SKILL_DIR` from the loaded `SKILL.md`; never assume the current
working directory contains a `skills/` checkout.
