<!-- MODIFIED IN THIS DERIVATIVE: validator commands are installation-safe and publication-path privacy is distinguished from task-deliverable links; see ../../../NOTICE (Apache-2.0 section 4(b)). -->
...
Copy the candidate into the task workspace before editing. Keep source assets
unchanged. Preserve license and attribution notices, but do not expose private
local paths or private template identifiers in generated figures, legends,
manuscript text, or publication-facing reports. A chat reply may link the
deliverables created for the current task without exposing unrelated private
sources.
...
2. Resolve `SKILL_DIR` from the loaded `SKILL.md`, then run
   `python3 "$SKILL_DIR/scripts/validate_figure.py" path/to/script.py` or the
