<!-- MODIFIED IN THIS DERIVATIVE: private-source paths are separated from links to current-task deliverables; see ../../../NOTICE (Apache-2.0 section 4(b)). -->
...
Do not reveal absolute paths, folder names, filenames, screenshots, provenance,
or identifying labels from the private source collection in publication-facing
output.

## Privacy rules

- Never embed private-source paths in generated code, public reports,
  publication artifacts, or manuscript text. A chat reply may link a
  deliverable created for the current task so the user can inspect it; that
  permission does not extend to unrelated private inputs or template locations.
...
Keep these commands in internal working notes only. Do not paste the user's
private template root into the final answer.
