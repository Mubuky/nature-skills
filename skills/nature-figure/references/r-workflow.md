<!-- MODIFIED IN THIS DERIVATIVE: single-backend scope now permits neutral output QA; see ../../../NOTICE (Apache-2.0 section 4(b)). -->
...
When the user has selected R, keep figure drawing, layout semantics, preview
generation, and export in R. Do not call Python/matplotlib/seaborn/plotly to
create a fallback rendering or layout approximation. Neutral image/PDF/SVG
viewers and deterministic validators may inspect R outputs. If R, `Rscript`, or
required R packages are missing, stop before rendering and report the missing
dependency. You may still write the R script, provide `install.packages()`
commands, or ask permission to install dependencies, but do not cross-render
the figure in another language.

Allowed non-R utilities include file and data inspection, checksums, archive
extraction, text search, and neutral viewing of generated image/vector files.
They must not redraw the figure or alter visual layout.
