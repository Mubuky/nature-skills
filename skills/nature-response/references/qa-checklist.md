...
- Load `action-mapping.md` and audit every action, work status, readiness label,
  verification-evidence field, and blocking state against its canonical
  definition.
- Confirm that required inputs and expected outputs are concrete enough for the
  author to act on and verify.
- Confirm that prose has not silently upgraded any tracker status.
...
- Formatting follows the target journal first. Reviewer page breaks are checked
  only when required by the journal, user, or print/PDF destination; ordinary
  Markdown is not failed for omitting them.
...
- Is package readiness derived from the canonical rules in
  `action-mapping.md`?
- Does each status-dependent claim cite the evidence required by
  `action-mapping.md`?

## Readiness gate

Treat `action-mapping.md` as the single source of truth for readiness labels,
work-status semantics, and package-level derivation. This checklist only
verifies that the selected label follows those rules.
