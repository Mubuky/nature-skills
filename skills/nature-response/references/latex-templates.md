...
7. Follow target-journal formatting first. Use
   `\ReviewerSection{1}`, `\ReviewerSection{2}`, etc. when switching reviewers,
   and redefine `\ReviewerSectionBreak` as `\clearpage` only when the journal,
   user, or print/PDF destination requires a new page.
...
- Reviewer section breaks match target-journal or user requirements; the
  template does not force a page break by default.
