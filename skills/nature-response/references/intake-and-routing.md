...
|---|---|---|---|
| `draft` | User wants a new point-by-point response | Reviewer comments plus any author actions or manuscript-change notes | Strategy summary, tracker, point-by-point draft, and risk flags; no cover letter, redline, or LaTeX unless requested |
...
| `revision-package` | User explicitly asks for a complete revision package | Decision email or reviewer/editor comments plus available author actions | Full combined package: strategy, tracker, response letter, cover letter, change checklist, and visible placeholders/risk flags; redline or LaTeX when in scope |
...
Use the canonical work-status and readiness definitions and derivation rules in
`action-mapping.md`; do not redefine them here. At intake, assign a provisional
state from the supplied evidence, then recalculate it after mapping and QA.
...
and resubmission instructions. If the email contains reviewer reports, enter
`revision-package` only when the user requested a full combined package;
otherwise enter `draft`. If reviewer boundaries are ambiguous, keep the original ordering visible and flag
...
Classify the missing fact and derive readiness under `action-mapping.md`.
...
- Vague author note such as "we fixed it" -> classify under
  `action-mapping.md`; do not infer verified completion.
