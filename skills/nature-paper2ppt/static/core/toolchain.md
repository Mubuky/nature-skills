...
This stack must work on macOS, Linux, and Windows. Use `pathlib` paths, the
user-requested output directory or a task-local fallback, and Office-safe fonts
or theme fonts. Never write task output into the installed skill directory. Do
not hardcode OS font paths or platform-specific file locations. If Python
packages are missing, create a local virtual environment and install the
minimum packages only when policy permits; do not install broad document
suites just to finish a normal deck.
...
5. Build the PPTX directly with python-pptx, using native tables/charts when
   values are explicit and figure crops when the original visual carries the
   evidence; retain the task-local generator code that reproduces it.
...
7. Verify by reopening the PPTX and inspecting package structure; render slide
   previews only if a reliable cross-platform headless renderer is already
   available, and report the canonical QA state without treating structural
   checks as visual review.
