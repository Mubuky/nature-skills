...
Transform a scientific paper or paper-derived notes into a complete,
figure-integrated PPTX presentation package with a Nature-style reporting logic.

The skill must not stop at an outline or script. The expected end product is a
real `.pptx` deck plus reusable task generator code. Keep other supporting
files minimal unless the user asks for more traceability.
...
Follow the user's requested language; otherwise follow the language of the
request. Preserve important technical terms, abbreviations, gene/protein names,
model names, dataset names, equations, and statistical terms in their clearest
canonical form, and keep them consistent via the Terminology Ledger.
