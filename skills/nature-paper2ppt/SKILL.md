...
1. Core principles (`core/principles.md`) — the argument is the spine; lean
   operating mode; accepted inputs; user-language rule.
...
Build the Terminology Ledger (`references/terminology-ledger.md`) while reading
the source, so model names, gene/protein names, datasets, metrics, and
abbreviations stay identical across every slide and speaker note.

Before creating files, parse any output directory named by the user. If none is
given, use a task-local `paper2ppt-output/` directory beside the task's source
or working files; never write task products into the installed skill
directory. Save the reusable task generator alongside the deck.
...
When a real PPTX has been generated, resolve `SKILL_DIR` to the directory
...
Treat high-severity findings as blockers, revise the deck, then re-run the
audit and record the final result and canonical QA state in
`$OUTPUT_DIR/qa_report.md`.
...
- The skill is self-contained so a single-skill install retains its terminology
  and QA contract.
