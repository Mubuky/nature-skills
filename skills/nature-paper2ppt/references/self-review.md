...
5. Re-run verification and update `$OUTPUT_DIR/qa_report.md` with what was
   checked, what was fixed, what remains, and the canonical QA state from
...
python3 "$SKILL_DIR/scripts/audit_pptx_quality.py" "$PPTX_PATH" \
  --report "$OUTPUT_DIR/pptx_audit.md" \
  --fail-on high
```

Resolve `SKILL_DIR` to the installed directory containing `SKILL.md`; do not
assume the current working directory is the repository root. If the script
reports high-severity findings, treat the deck as not ready. Fix the deck,
regenerate, and run the script again. Copy the final audit summary into
`$OUTPUT_DIR/qa_report.md`. This audit reports `structural_qa_only`; it cannot
establish a rendered visual pass.
...
If no reliable renderer is available, still complete the self-review loop
using crop/contact-sheet inspection for selected assets,
`audit_pptx_quality.py`, and slide-by-slide text and shape inspection. Record
`structural_qa_only` in `$OUTPUT_DIR/qa_report.md`; never describe this as
visual QA passed. If rendered visual QA was explicitly required but cannot be
completed reliably, record `visual_qa_blocked` instead.

If reliable final previews are available, inspect every slide after the last
revision. Record `rendered_visual_qa_passed` only when the structural audit and
that final rendered inspection both pass. If rendering reveals an unresolved
high-severity visual defect, record `visual_qa_blocked`.
...
Do not stop at "PPTX opens" if the self-review found high-severity issues.
Correct them first, then verify again. Document any remaining limitation and
exactly one canonical QA state in `$OUTPUT_DIR/qa_report.md`.
