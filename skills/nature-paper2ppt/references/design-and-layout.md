...
Use
`python3 "$SKILL_DIR/scripts/audit_pptx_quality.py" "$PPTX_PATH"` after deck
generation to catch shape bounds, repeated near-miss alignment, and text
overload. The script reports structural QA only; it is not a substitute for
visual review, and any high-severity finding should trigger revision.
