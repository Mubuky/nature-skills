...
Repository planning and local package preparation are read-only by default.
Before any external create/upload/publish/update action, confirm the exact
repository/account, file list and hashes, visibility/embargo, licence, rights
and consent, metadata preview, DOI-mint timing, and recovery/versioning path.
Never treat a request to draft a statement or prepare source data as upload
authorization. Treat reviewer links and access tokens as secrets; do not echo
them or place them in ordinary logs.
...
- This structure mirrors `nature-writing`, `nature-figure`, `nature-citation`,
  and `nature-response`.
