...
Follow `static/core/workflow.md`: segment claims, discover metadata candidates,
verify identity and scope, screen semantic support, and only then export a
reference-manager file. Candidate discovery is useful intermediate work, but it
is not a completed citation result and must not produce insertion markers or
import-ready records. Prefer `scripts/nature_citation.py` for discovery and
validated export when internet access is available; open
`references/script-usage.md` for its full flag list and long-article strategy.

Never present a paper as support merely because its title is related. Require
an auditable evidence locator, precise paraphrase or bounded excerpt, check URL
and time, and resolved contradiction/retraction status before final export. Do
not invent missing bibliographic fields.
