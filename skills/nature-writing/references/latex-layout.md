...
Preferred loop when a suitable toolchain is available and compilation is in
scope: **change → compile → render to image → look → iterate.** When compilation
or rendering is unavailable, inspect the source and available artifacts, report
`visual_qa_not_run`, and do not claim that the rendered layout passed. Measure
what is available; do not guess.

---

## 0. Diagnosis workflow (do this first)

1. First determine whether the relevant LaTeX toolchain, assets, and compilation
   permission are available. If they are, compile and read the log for the three
   signals that matter:
...
2. When a PDF can be built or is supplied, render pages to PNG and *look*. With pymupdf:
...
- [ ] If compilation was available, compile clean: no "Float too large", no
      "Overfull \vbox", no undefined refs.
- [ ] If rendering was available, contact-sheet the final PDF and inspect every
      page (§0); otherwise report `visual_qa_not_run`.
