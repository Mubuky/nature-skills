<!-- MODIFIED IN THIS DERIVATIVE: word-budget guidance is now diagnostic rather than a mandatory confirmation gate; see ../../../../../NOTICE (Apache-2.0 section 4(b)). -->

# Journal: Nature Communications (writing)

## Read the shared facts first

Open `../../../references/nat-comms-format.md` for the local formatting snapshot:
word limits, abstract rules, figure specs, reference style, mandatory statements,
and common desk-rejection patterns.
...
The local early-2026 snapshot records an approximately 5,000-word Article
budget including Methods. For a full manuscript or a draft near the limit,
estimate the current allocation and report a proposed budget; do not impose a
separate confirmation round for a well-scoped section request.
...
If the user already has a near-complete manuscript, estimate the current split;
if Methods runs above the proposed allocation, name it as a budget risk rather
than silently compressing reproducibility detail.
