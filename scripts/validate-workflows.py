...
"""Validate the single GitHub Actions repository-health workflow.

The release repository intentionally keeps one workflow. If path filters are
added later, this check also requires every locally invoked validator or shell
script to appear in both pull_request and push filters.
"""
...
def main() -> int:
    ...
    if names != {"validate.yml"}:
        errors.append(
            ".github/workflows: expected only validate.yml, found "
            + ", ".join(sorted(names))
        )
