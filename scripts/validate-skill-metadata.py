...
def check_manifest_path(manifest_path: Path, skill_dir: Path, raw_path: str) -> str | None:
    ...
    if ".." in candidate.parts:
        return f"{manifest_path.relative_to(ROOT)}: referenced path may not contain '..': {raw_path}"

    target = (skill_dir / candidate).resolve()
    try:
        target.relative_to(skill_dir.resolve())
    except ValueError:
        return f"{manifest_path.relative_to(ROOT)}: referenced path escapes its skill directory: {raw_path}"
...
def main() -> int:
    ...
    triggerable_count = len(skill_dirs)
